...
if (room == rMenu and D.game_state == GAME.MENU) {
	...
	#region Pre Menu Vox
		...
		if((bgmID == N or !audio_is_playing(bgm)) and preDel <= 0 and M.introInst == N) {
			...
			audio_sound_gain(bgmID,bgmVol,4000)
	...
	#region Title Prep
		...
		if(titSurf == N or !surface_exists(titSurf) or surface_get_width(titSurf) != WW or surface_get_height(titSurf) != WH) {
			...
			draw_text_transformed_color(WW/1.95,WH*.19,_str,(WW/1.9)/_strw,(WW/1.9)/_strw,titAngl,c.gry,c.gry,c.dgry,c.dgry,2/3)
	...
	#region Buttons
		...
		if(fade <= 0 and M.introInst == N) {
			...
			if(titDel > 0) titDel--;
			if(btnDel > 0 and titDel == 0) btnDel--;
